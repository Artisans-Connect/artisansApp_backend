...
export async function createJob(userId: string, body: unknown, idempotencyKeyHeader?: string) {
  ...
  const { data, error } = await supabaseAdmin
    ...
    .insert({
      ...
      job_archetype: input.job_archetype ?? "fixed_scope",
      budget_type: input.budget_type,
      budget_fixed: input.budget_fixed ?? null,
      budget_min: input.budget_min ?? null,
      budget_max: input.budget_max ?? null,
      diagnostic_fee: input.diagnostic_fee ?? null,
      scheduled_for: input.scheduled_for ?? null,
      service_type: input.service_type,
      landmark_description: input.landmark_description ?? null,
      milestone_stages: input.milestone_stages ?? null,
      archetype_payload: input.archetype_payload ?? null,
